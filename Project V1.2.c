...
//TO DO:
//1)FIX SAMPLE DIALOGUE
//2)IMPLEMENT MEDIAN
//3)DO A LOWEST TO HIGHEST MARK?
//4)(only if above is done) histogram of marks?(something like bell curve)
...
int main(void)
{
    ...
    //INPUT mark, PROCESS student's grade
    ...
    //PROCESS for average of student's mark
    ...
    printf("\n\n%-10s%20s%24s%10s%10s\n", "Name", "AttendanceRate","Percentage","Grade","Status");
    ...
    for(int f = 0; f != numberOfStudents; f++)
    {
        ...
        {
            printf("%-10s      %-20.2f        %10.2f     %5c       %s  \n", student[f].name, student[f].attendanceRate, student[f].mark,student[f].grade,"DEBARRED");
        }
        else
        {
            printf("%-10s      %-20.2f        %10.2f     %5c        \n",student[f].name, student[f].attendanceRate, student[f].mark,student[f].grade);
        }
    }
